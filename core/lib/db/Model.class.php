...
class Model {
    ...
    protected function parseSql($sql,$parse) {
        ...
        if(true === $parse) {
            ...
            $sql    =   preg_replace_callback("/__([A-Z0-9_-]+)__/sU", 'getFullName', $sql);
        }
        $this->db->setModel($this->name);
        return $sql;
    }
    ...
    public function db($linkNum='',$config='',$force=false) {
        ...
        if(!isset($this->_db[$linkNum]) || $force ) {
            ...
            $this->_db[$linkNum]            =    thinkphpDb::getInstance($config);
        }elseif(NULL === $config){
    ...
    public function table($table) {
        ...
        if(is_array($table)) {
            ...
            $table  = preg_replace_callback("/__([A-Z0-9_-]+)__/sU", 'getFullName', $table);
            $this->options['table'] =   $table;
        }
        return $this;
    }
    ...
    public function using($using){
        ...
        if(is_array($using)) {
            ...
            //将__TABLE_NAME__替换成带前缀的表名
            $using  = preg_replace_callback("/__([A-Z0-9_-]+)__/sU", 'getFullName', $using);
            $this->options['using'] =   $using;
        }
        return $this;
    }
    ...
    public function join($join,$type='INNER') {
        ...
        if(is_array($join)) {
            foreach ($join as $key=>&$_join){
                $_join  =   preg_replace_callback("/__([A-Z0-9_-]+)__/sU", 'getFullName', $_join);
                $_join  =   false !== stripos($_join,'JOIN')? $_join : $type.' JOIN ' .$_join;
            }
            $this->options['join']      =   $join;
        }elseif(!empty($join)) {
            //将__TABLE_NAME__字符串替换成带前缀的表名
            $join  = preg_replace_callback("/__([A-Z0-9_-]+)__/sU",'getFullName', $join);
    ...
    public function union($union,$all=false) {
        ...
        if(is_string($union) ) {
            //$prefix =   $this->tablePrefix;
            //将__TABLE_NAME__字符串替换成带前缀的表名
            $options  = preg_replace_callback("/__([A-Z0-9_-]+)__/sU", 'getFullName', $union);
